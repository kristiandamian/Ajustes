...
def AjustesSinFinalizar(request):
    template = loader.get_template('reportes/AjustesSinFinalizar.html')
    ...
    if request.method == 'POST':
        ...
        if not error:
            if tda>0:
                datos=Ajuste.objects.filter(Finalizado=False,Activo=True,Tienda=tda, FechaRecepcion__gte=fechainicial, FechaRecepcion__lte=fechafinal).order_by("FechaRecepcion","Tienda")
            else:
                if int(region)>0:
                    datos=Ajuste.objects.filter(Finalizado=False,Activo=True,Region__id=region, FechaRecepcion__gte=fechainicial, FechaRecepcion__lte=fechafinal).order_by("FechaRecepcion","Tienda")
                else:
                    datos=Ajuste.objects.filter(Finalizado=False,Activo=True,FechaRecepcion__gte=fechainicial, FechaRecepcion__lte=fechafinal).order_by("FechaRecepcion","Tienda")
